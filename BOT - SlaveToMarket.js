...
//SlvToMa
RoomName = "SlvToMa"
RoomDescription = "\"Slaves to Market\" best offers in town. Read the bot profile for instructions!"
...
Player.Description = `
    ....... automated ServiceBot model "Slave Trader" 0.0.0.8 .......
        Slave to Market
        =============
        ...
        How it works
        -----------------------
        Mistress offers valuable candidates as merchandising item.  To become a candidate someone can offer herself (#sellme)  or someone suggest to sell anotger one (#sell someone)
        if the candidates doesn't met the requirements they will kicked out. 
        Anyone who misbehaves e.g. using Mistress commands will be remembered by the Mistress. She will reward that at punishment times.
        Items on he shelf will get a price and a selling description. Ask Mistress for that informations about someone (#present someone).
...
function newGame() {
    ...
    setTimeout(function (Player) { checkSlave() }, Math.floor(Math.random() * csrPunishment + csPunishment))
}
// Checker

function isTrader(memberNumber) {
    if (memberNumber in guestList && guestList[memberNumber].role == "Trader")
        return true
    else
        return false
}
...
function isMerchandise(memberNumber) {
...
function welcomeGreetings(sender) {
    ...
    console.log(charname(sender) + " Welcome Greetings")
}
function checkGuest(sender) {
    ...
    if (isMerchandise(sender.MemberNumber)) {
        ...
        if (isSlave(sender.MemberNumber)) {
            answer = "You stay where you belong"
            sendAnswer(sender, answer)
        }
        ...
        if (guestList[sender.MemberNumber].role == "")
            rolechange(sender.MemberNumber, "customer")
    }
}
...
function checkMerchandise(sender) {
    ...
    if (memberNumber in guestList)
        if (isMerchandise(memberNumber)) {
            ...
            setTimeout(function (Player) { prepareMerchandise(Player, sender, [], 1) }, 8 * 1000)
        }
    if (isSlave(memberNumber)) {
        memorizeClothing(sender)
        ServerSend("ChatRoomChat", { Content: " waves to " + charname(sender), Type: "Emote", });
    ...
    if (data.Type != null && sender.MemberNumber != Player.MemberNumber) {
        if (msg.startsWith("ActionUse") || msg.startsWith("ChatOther")) {
            ...
            if (target == Player.MemberNumber) {
                ...
                if (isTrader(Player.MemberNumber)) {
                    ...
                    ServerSend("ChatRoomChat", { Content: "*Thank you, Please go on!", Type: "Whisper", Target: sender.MemberNumber });
                }
            }
            if ((sender.MemberNumber in guestList) && (target in guestList)) {
                if (isSlave(sender.MemberNumber)) {
                    ServerSend("ChatRoomChat", { Content: "*As slave you are not allowed to touch anybody. I add a punishment point to your score.", Type: "Whisper", Target: sender.MemberNumber });
                    guestList[sender.MemberNumber].punishmentPoints++;
                }
                else {
                    if (isSlave(target.MemberNumber)) {
                        ...
                        if (isCustomer(sender.MemberNumber)) {
                            if (isMerchandise(target)) {
                                ServerSend("ChatRoomChat", { Content: "*Feel free to test our merchandising ", Type: "Whisper", Target: sender.MemberNumber });
                            }
                            if (isCustomer(target)) {
                                ServerSend("ChatRoomChat", { Content: "*Please concentrate on buying merchandising items", Type: "Whisper", Target: sender.MemberNumber });
                            }
                        }
                        else
                            if (isMerchandise(sender.MemberNumber)) {
                                if (isMerchandise(target)) {
                                    ServerSend("ChatRoomChat", { Content: "*be careful, don't damage the product.", Type: "Whisper", Target: sender.MemberNumber });
                                }
                                if (isCustomer(target)) {
                                    ServerSend("ChatRoomChat", { Content: "*You are not allowed to touch noble customers. I add a punishment point to your score.", Type: "Whisper", Target: sender.MemberNumber });
                                    guestList[sender.MemberNumber].punishmentPoints++;
                                }
                            }
...
function TraderCommandHandler(sender, msg) {
    ...
    if (msg.toLowerCase().includes("enslave")) {
        console.log("enslave  :" + sender.MemberNumber)
        playerList = []
        playerList = extractNumberfromMessage(sender, msg)
        prepareSlaves4enslavement(sender, playerList, 0)
...
function commandHandler(sender, msg) {
    ...
    if (msg.toLowerCase().includes("sell ")) {
        ...
        if (isCustomer(sender.MemberNumber)) {
            ...
            answer = "*You are not allowed to sell someone."
            sendAnswer(sender, answer)
        }
    }
    if (msg.toLowerCase().includes("enslave ")) {
        console.log("enslave command from " + sender.MemberNumber)
        if (isCustomer(sender.MemberNumber)) {
            checkGuest(sender)
            playerList = extractNumberfromMessage(sender, msg)
            prepareSlaves4enslavement(sender, playerList, 0)
            ...
            answer = "*You are not allowed to enslave someone."
            sendAnswer(sender, answer)
        }
    }
    ...
    if (msg.toLowerCase().includes("buy")) {
        ...
        sellernumber = findNumberfromCharname(sellername)
         if (!((sellernumber in guestList) && isMerchandise(sellernumber)) ){
        ...
        if ((sellernumber in guestList) && isMerchandise(sellernumber)) {
            ...
            answer = "*You are not allowed to buy anyone."
            sendAnswer(sender, answer)
        }
    }
    ...
    function playerUpdate() { ChatRoomCharacterUpdate(Player) }
    ...
    if (msg.includes("inspect")) {
        ...
        for (memberNumber in guestList) {
            if (memberNumber != Player.MemberNumber && isMerchandise(guestList[memberNumber])) {
    ...
    if (msg.toLowerCase().includes("info")) {
        ...
        if (standingTime != null)
            mess = mess + nl + "You remain in that role since " + standingTime + ` minutes  !`;
        ...
        mess = mess + "*--------------------"
        if (isMerchandise(sender.MemberNumber))
            mess = mess + nl + "There are " + game.customerCount() + " potential buyer(s)   "
        else
            if (isSlave(sender.MemberNumber))
    ...
    if ((msg.toLowerCase().includes("buggy") && isCustomer(sender.MemberNumber))) {
        //punish bot
        dressColor = ""
        // own haicolor
        ...
        InventoryGet(Player, "ItemHood").Property = { "Type": "e3m2b2br2op2ms0", "Difficulty": 15, "Effect": ["BlindHeavy", "Prone", "BlockMouth"], "Hide": ["Glasses", "ItemMouth", "ItemMouth2", "ItemMouth3", "Mask", "ItemHead"], "HideItem": ["ItemHeadSnorkel"] }
        //ChatRoomCharacterUpdate(Player);
        guestList[Player.MemberNumber].role = "caught Trader"
        setTimeout(function (Player) { playerUpdate() }, Math.floor(Math.random() * 1000 + 1000, Player))
        ServerSend("ChatRoomChat", { Content: "I am buggy, please punish me", Type: "Chat" });

    }
    if ((msg.toLowerCase().includes("buggy") && !isCustomer(sender.MemberNumber))) {

        ServerSend("ChatRoomChat", { Content: "Bad news, you are no customer.", Type: "Chat", Target: sender.MemberNumber });
        guestList[sender.MemberNumber].punishmentPoints++;
        InventoryWear(sender, "PaddedBlindfold", "ItemHead", "Default", 15)
        ChatRoomCharacterUpdate(sender);
...
function checkGuests() {
        ...
        if (ChatRoomCharacter[D].MemberNumber != Player.MemberNumber) {
            if (!(ChatRoomCharacter[D].MemberNumber in guestList)) {
                ...
                if (guestList[ChatRoomCharacter[D].MemberNumber] != null)
                    if (guestList[ChatRoomCharacter[D].MemberNumber].role == null)
                        rolechange(ChatRoomCharacter[D].MemberNumber, "customer")
    ...
    memberNumber = 0
    for (memberNumber in guestList) {
        if (actualList.indexOf(Number(memberNumber)) == -1)
            delete guestList[memberNumber]
    }
}
...
function tellHelp(char) {
    ...
    answer = answer + 'Any one who doesn`t met the requiremnts will be kicked out.' + nl
    answer = answer + 'Don`t touch the Mistress' + nl
    answer = answer + 'If there are enough items on shelf, Mistress will park some slaves aside. ' + nl
    answer = answer + '********************************' + nl
...
function tellRelease(char) {
    if (isMerchandise(char.MemberNumber)) {
...
function analyzeCharacter(sender) {
    ...
    if (ReputationCharacterGet(sender, "Maid") > 75) {
        description = description + "She is a trained maid, ready to serve." + nl
    }
    if (ReputationCharacterGet(sender, "Gambling") > 50) {
        description = description + "She is a gambler, ready to play for her freedom." + nl
    }
    if (ReputationCharacterGet(sender, "LARP") > 50) {
        description = description + "A LARP loser" + nl
    }
    if (ReputationCharacterGet(sender, "Asylum") > 3) {
        description = description + "She was educated in Asylum" + nl
    }
    if (ReputationCharacterGet(sender, "Gaming") > 3) {
        description = description + "She is a game Player" + nl
    }
    if (ReputationCharacterGet(sender, "HouseCorporis") > 33) {
        description = description + "She is a member of HouseCorporis." + nl
    }
    return description
}
...
function canSell(sender, char) {
    if (guestList[char.MemberNumber].role == "merchandise") {
        guestList[char.MemberNumber].Description = analyzeCharacter(char)
        return false
    }
    else
        return true
    ...
    if (playerList.length > 0) {
        ...
        if (char == null) {
            ...
            if (warnmsg != "ok") {
                ...
                warnmsg = "Your item is not prepered: i advised her :  " + warnmsg
                ...
                if (guestList[char.MemberNumber].role == "")
                    sup = "unknown"
                else
                ...
                if (canSell(sender, char)) {
                    ...
                    rolechange(char.MemberNumber, "merchandise")
                    enslave(char)
                    ServerSend("ChatRoomChat", { Content: " waves to " + charname(char), Type: "Emote", });
                    if (guestList[char.MemberNumber].role == "customer")
                        memorizeClothing(char)
                    ChatRoomCharacterUpdate(char);
                    newSlaves += 1
                    setTimeout(function (Player) { prepareMerchandise(sender, char, playerList, newSlaves) }, 8 * 1000)
                } else {
                    guestList[sender.MemberNumber].punishmentPoints += 1
...
function prepareMerchandise(sender, char, playerList, newSlaves) {
    ...
    ChatRoomCharacterUpdate(char);
    setTimeout(function (Player) { prepareMerchandise2(sender, char, playerList, newSlaves) }, 8 * 1000)
}
function prepareMerchandise2(sender, char, playerList, newSlaves) {
    ...
    ChatRoomCharacterUpdate(char);
    setTimeout(function (Player) { prepareMerchandise3(sender, char, playerList, newSlaves) }, 8 * 1000)
}
function prepareMerchandise3(sender, char, playerList, newSlaves) {
    if ((char != null) && (char.MemberNumber in guestList)) {
        ...
        ChatRoomCharacterUpdate(char);
        setTimeout(function (Player) { prepareMerchandise4(sender, char, playerList, newSlaves) }, 8 * 1000)
    }
}
function prepareMerchandise4(sender, char, playerList, newSlaves) {
    InventoryWear(char, "LeatherMittens", "ItemHands", "202020", 50)
    //InventoryWear(char, "LeatherToeCuffs", "ItemBoots", "#000000", 50)
    ...
    for (var D = 0; D < ChatRoomCharacter.length; D++) {
        ...
        if (slaveRow && (isMerchandise(ChatRoomCharacter[D].MemberNumber) || isSlave(ChatRoomCharacter[D].MemberNumber)))
            targetPos++
        else {
            slaveRow = false
        }
...
function prepareSlave2(sender, char, playerList, newSlaves) {
    ...
    InventoryWear(char, "SturdyLeatherBelts", "ItemFeet", "#000000", 50)
    InventoryWear(char, "LeatherChastityBelt", "ItemPelvis", "#000000", 50)
    ServerSend("ChatRoomChat", { Content: " loves humiliating slaves", Type: "Emote", });
    ChatRoomCharacterUpdate(char);
...
function prepareSlave3(sender, char, playerList, newSlaves) {
    if ((char != null) && (char.MemberNumber in guestList)) {
        ...
        //CharacterSetActivePose(char, "Kneeling", true)
        //ServerSend("ChatRoomCharacterPoseUpdate", { Pose: char.ActivePose });
        ServerSend("ChatRoomChat", { Content: " looking for a kennel", Type: "Emote", });
        ChatRoomCharacterUpdate(char);
...
function prepareSlave4(sender, char, playerList, newSlaves) {
    InventoryWear(char, "CollarChainShort", "ItemNeckRestraints", "Default", 50)
    InventoryWear(char, "Kennel", "ItemDevices", ("#202020", "#780E0E", "#202020", "#202020"), 50)
    InventoryGet(char, "ItemDevices").Property = { "Type": "d1p1", "door": true, "Padding": true }
    ServerSend("ChatRoomChat", { Content: " knock on the kennel and turn away from " + charname(char), Type: "Emote", });
    //move
    var targetPos
    var sourcePos
    sourcePos = 0
    ...
    personContent = convertPers(char)
    saveCharResult(memberNumber, personContent, gamekey)
    setTimeout(function (Player) { prepareSlaves4enslavement(sender, playerList, newSlaves) }, 1000)
}
...
function prepareSlaves4enslavement(sender, playerList, newSlaves) {
    if (playerList.length > 0) {
        ...
        if (char == null) {
            ...
            warnmsg = checkRequirements(char)
            if (warnmsg != "ok") {
                console.log(charname(sender) + "Enslavement requirements failed")
                ServerSend("ChatRoomChat", { Content: warnmsg, Type: "Whisper", Target: char.MemberNumber });
                warnmsg = "Your item is not prepered: i advised her :  "
                ServerSend("ChatRoomChat", { Content: warnmsg, Type: "Whisper", Target: sender.MemberNumber });
                setTimeout(function (Player) { ChatRoomAdminChatAction("Kick", char.MemberNumber.toString()) }, 6 * 1000)
                setTimeout(function (Player) { prepareSlaves4enslavement(sender, playerList, newSlaves) }, Math.floor(Math.random() * 2000 + 150))
            }
                ...
                if (delinquent == sender.MemberNumber)
                    answer = "Up to now you was a " + sup + nl
                else
                    answer = "Someone wants you to be enslaved. Up to now you was a " + sup + nl
                if (canSell(sender, char)) {
                    rolechange(char.MemberNumber, "slave")
                    enslave(char)
                    ServerSend("ChatRoomChat", { Content: " turns to " + charname(char), Type: "Emote", });
                    // memorizeClothing(char)
                    ChatRoomCharacterUpdate(char);
                    newSlaves += 1
...
function releaseSlave(slaveObj) {
    ...
    if (memberNumber in guestList) {
        rolechange(memberNumber, 'customer')
    ...
    reapplyClothing(slaveObj, false)
...
function presentSlaves(sender, playerList) {
    if (playerList.length > 0) {
        ...
        if (isMerchandise(delinquent))
            presentSlave(sender, char, playerList)
        else {
            if (isSlave(delinquent)) { answer = charname(char) + " is enslaved and not on market!" }
            else {
                answer = charname(char) + " is customer!"
            }
            sendAnswer(sender, answer)
...
function presentSlave(sender, char, playerList) {
    ...
    if (char.IsFullyOwned()) {
        answer = answer + "Slave already full owned by "
        answer = answer + char.OwnerName() + " - " + char.OwnerNumber()
    }
    else if (Number(char.OwnerNumber()) > 0) { answer = answer + "Trial owned slave by " + char.OwnerName() + " -" + char.OwnerNumber() }
    else
        answer = answer + "..an unowned gem."
...
function soldSlave2(buyer, item, dressColor) {
    ...
    // change role 
    rolechange(item.MemberNumber, "sold")
...
function reconvertPers(personContent, char) {
    ...
    if (personContent.name != "") {
        ...
        rolechange(char.MemberNumber, personContent.role)
...
function checkSlave() {
    ...
    checkGuests()
    if (!isTrader(Player.MemberNumber)) {
        ServerSend("ChatRoomChat", { Content: "is helpless, need mercy", Type: "Emote" });
        setTimeout(function (Player) { checkSlave() }, Math.floor(Math.random() * csrPunishment + csPunishment))
        return true
    }
    if (game.status == 'offering') { game.status = 'punishment' }
    else
        ...
        if (ChatRoomCharacter[D].MemberNumber in guestList)
            if (guestList[ChatRoomCharacter[D].MemberNumber].punishmentPoints > 0) {
                ...
                if (isCustomer(ChatRoomCharacter[D].MemberNumber)) {
                    ...
                    InventoryWear(ChatRoomCharacter[D], "CeilingShackles", "ItemArms", ["##C6D3E0", "#403E40", "#C6D3E0", "#403E40", "#11161B", "#403E40"], 50)
                ...
                if (isMerchandise(ChatRoomCharacter[D].MemberNumber)) {
                ...
                ServerSend("ChatRoomChat", { Content: "You earned punishment.", Type: "Whisper", Target: ChatRoomCharacter[D].MemberNumber });
    ...
    if (countps == 0) {
        if (guestList.length == 1)
            ...
            setTimeout(function (Player) { CheckRewardApplied() }, Math.floor(Math.random() * 9000 + 6000))
        }
    }
    else {
        ...
        //punishment crop
        ChatRoomCharacterUpdate(Player)
        setTimeout(function (Player) { CheckSlavePunishment() }, Math.floor(Math.random() * 1600 + 10000, Player))
    }
}
function CheckSlavePunishment() {
     console.log("CheckSlavePunishment, time : " + timestamp(new Date()))
      var emptyGuest = 0
    for (memberNumber in guestList) {
        char = charFromMemberNumber(memberNumber)
        if (char == null) 
            ...
            console.log("CheckSlavePunishment - for " + memberNumber)
            if (guestList[memberNumber].beingPunished) {
                if (guestList[memberNumber].punishmentPoints == 0) {
                    ...
                    guestList[memberNumber].punishmentPoints--
                    if (isMerchandise(memberNumber)) {
                        rebindMerchandise(char)
                    }
                }
                else {
                    if (guestList[memberNumber].punishmentPoints <= -2) {
                        // Punishement is done
                    } else {
                        emptyGuest++
                ...
                // no punishment, good girl 
                if (guestList[memberNumber].punishmentPoints == -1) {
                    if (isMerchandise(memberNumber)) {
                        emptyGuest++
    ...
    if (emptyGuest == 0) {
        ...
        setTimeout(function (Player) { CheckSlavePunishment() }, Math.floor(Math.random() * 10000 + 1000))
    return true
}

function rebindMerchandise(char) {
...
function checkSellingStatus() {
    console.log("checkSellingStatus, time : " + timestamp(new Date()))
    game.status = "checkingStatus"
    var checkGuestList = []
    for (memberNumber in guestList) {
...
function checkSellingSingleStatus(checkGuestList) {
    console.log(checkGuestList)
    actdelinquent = checkGuestList.shift()
    var char = charFromMemberNumber(actdelinquent)
    ...
    if (char == null) {
        ...
        if (isMerchandise(actdelinquent)) {
            removeRestrains(char)
            if (game.merchandiseCount() > 1 || game.customerCount() == 0) {
                //merchandiseList = []
                console.log("shelfwarmer, time : " + timestamp(new Date()))
                rolechange(actdelinquent, "slave")
                ServerSend("ChatRoomChat", { Content: " stand behind " + charname(char), Type: "Emote", });
                newSlaves = 0
                //merchandiseList.push(char.MemberNumber)
                setTimeout(function (Player) { prepareSlave(Player, char, [], newSlaves) }, Math.floor(Math.random() * 10000 + 2000))
            }
            else {
                //backup update     
                console.log("stays on shelf, time : " + timestamp(new Date()))
                InventoryWear(char, "LeatherMittens", "ItemHands", "202020", 50)
                InventoryWear(char, "SturdyLeatherBelts", "ItemArms", ["#11161B", "#403E40", "#11161B", "#403E40", "#11161B", "#403E40"], 50)
                InventoryLock(char, InventoryGet(char, "ItemArms"), { Asset: AssetGet("Female3DCG", "ItemMisc", "CombinationPadlock") }, Player.MemberNumber)
                InventoryGet(char, "ItemArms").Property.CombinationNumber = guestList[char.MemberNumber].lockCode
            ...
            if (isSlave(char.MemberNumber)) {
                if (game.customerCount() > 0) {
                    console.log("is presented on shelf, time : " + timestamp(new Date()))
                    removeRestrains(char)
                    rolechange(actdelinquent, "merchandise")
                    ChatRoomCharacterUpdate(char)
                    ServerSend("ChatRoomChat", { Content: " waves to " + charname(char), Type: "Emote", });
                    newSlaves = 0
                    merchandiseList = []
                    setTimeout(function (Player) { prepareMerchandise(Player, char, [], newSlaves) }, Math.floor(Math.random() * 10000 + 2000))
                }
                else {
                    console.log("remains as slave, time : " + timestamp(new Date()))
                }
            }
        }
    }
...
function checkTime(memberNumber) {
    actTime = new Date()
    oldTime = guestList[memberNumber].from
    let diff = actTime - oldTime;
    let difmin = Math.floor(diff / 1000 / 60) % 60;
    bolret = false
    if (difmin > ctMin) {
        bolret = true
    }
...
function rolechange(memberNumber, role) {
    guestList[memberNumber].role = role
    guestList[memberNumber].from = new Date()
}
